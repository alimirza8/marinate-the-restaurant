...
/**
 * Renders northlife-church Flexible Content
 *
 * @param  string $template The northlife-church Flexible template
 */
...
/**
 * echo northlife-church field with HTML Markup
 *
 * @param  string $name         northlife-church Object
 ...
 * @return Mixed                render northlife-church Object
 */
...
/**
 * Prints an anchor tag from an array of northlife-church Link object
 */
function ja_the_link($link = array(),  $classes = '', $target = '', $before = '', $after = '', $attributes = '')
{
  ...
  if (!strpos($attributes, 'aria-label')) {
    $attributes .= ' aria-label="' . wp_sprintf(__('Open for %s', 'northlife-church'), strip_tags($link['url'])) . '"';
  }
...
?>
    <div class="iframe-container <?php echo $custom_classes; ?>">
      <iframe width="100%" src="https://www.youtube.com/embed/<?php echo $youtube_vars['v'] . '?' . $video_attributes; ?>" title="<?php echo wp_sprintf(__('Youtube Video ID: %s', 'northlife-church'), $youtube_vars['v']); ?>" allow="encrypted-media" allowfullscreen></iframe>
    </div>
  <?php
  ...
  ?>
    <div class="iframe-container <?php echo $custom_classes; ?>">
      <iframe width="100%" src="https://player.vimeo.com/video/<?php echo $vimeo_id . '?' . $video_attributes; ?>" title="<?php echo wp_sprintf(__('Vimeo ID: %s', 'northlife-church'), $vimeo_id); ?>" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>
    </div>
  <?php else : ?>
